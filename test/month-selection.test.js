...
const renderMonthSelector = (shouldOpenSelector = true, props) => {
  const { getAllByLabelText, getByTestId, rerender, getByText } = render(
    <Calendar {...props} />,
  )
...
describe('Month Selection', () => {
  ...
  test('renders all months', () => {
    ...
    fireEvent.click(monthButton)
    const persianMonthTexts = Array.from(monthSelector.children).map(
  ...
  test('highlights the initial active month', () => {
    ...
    const activeMonth = monthsChildren.find(child =>
  ...
  test('selects a new month', () => {
    ...
    const nonSelectedMonth = monthsChildren.find(
      child => !child.classList.contains('-active'),
    )
    const selectedMonth = monthsChildren.find(child =>
      child.classList.contains('-active'),
    )
  ...
  test('disables months according to minimum & maximum dates', () => {
    const { rerender, getByText, calendarGetByText } = renderMonthSelector(
      true,
      {
  ...
  test('navigates by keyboard', () => {
    ...
    const { getByText, monthSelectorWrapper } = renderMonthSelector(true, {
      value,
    })

...
const renderOpenDatePicker = (renderProps = {}) => {
  const { getByTestId, ...otherSelectors } = render(
    <DatePicker {...renderProps} />,
  )
...
describe('DatePicker Visibility', () => {
  ...
  test('renders the datepicker with custom locale prop', () => {
    // ltr language
    expect(
      render(<DatePicker value={null} onChange={() => {}} locale="en" />)
        .container,
    ).toStrictEqual(
      render(
        <DatePicker
    ...
    // rtl language
    expect(
      render(<DatePicker value={null} onChange={() => {}} locale="fa" />)
        .container,
    ).toStrictEqual(
      render(
        <DatePicker

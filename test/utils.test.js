...
describe('Utility Functions', () => {
  describe('General Utilities', () => {
    test('returns the next or previous month according to date', () => {
      ...
      expect(getDateAccordingToMonth(thisMonth, 'PREVIOUS')).toEqual(
        previousMonth,
      )
    })
  ...
  describe('Language Dependent Utilities', () => {
    ...
    test('returns correct language digits', () => {
      ...
      expect(gregorianUtils.getLanguageDigits(englishDigits)).toBe(
        englishDigits,
      )
    ...
    test('returns the first weekday of the passed month', () => {
      ...
      expect(
        persianUtils.getMonthFirstWeekday(persianMonthStartingWithSaturday),
      ).toBe(0)
    })
    ...
    test('returns whether a day is before another', () => {
      ...
      expect(gregorianUtils.isBeforeDate(gregorianDay1, gregorianDay2)).toBe(
        true,
      )
      expect(gregorianUtils.isBeforeDate(gregorianDay2, gregorianDay1)).toBe(
        false,
      )
    ...
    test('returns if a day is in a range', () => {
      ...
      expect(
        gregorianUtils.checkDayInDayRange({ ...range, day: dayInRange }),
      ).toBe(true)
      expect(
        gregorianUtils.checkDayInDayRange({ ...range, day: dayNotInRange }),
      ).toBe(false)
    })
  })
})

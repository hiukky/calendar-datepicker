...
const flatDays = section =>
  Array.from(section.children).reduce(
    (previous, next) => [...previous, ...next.children],
    [],
  )
...
const renderCalendar = props => {
  ...
  const sections = Array.from(
    selectors.getByTestId('days-section-wrapper').children,
  )
  const monthYearItems = Array.from(
    selectors.getByTestId('month-year-container').children,
  )
  const endSlideAnimation = () => {
    const findAnimatedChild = child =>
...
describe('Calendar Days', () => {
  describe('Basic Rendering', () => {
    ...
    test(`shows current month and year title when there's no value`, () => {
      ...
      const shownMonthYear = Array.from(
        getByTestId('month-year-container').children,
      ).find(child => child.classList.contains('-shown'))
    ...
    test(`shows date's month and year as title when it has value`, () => {
      ...
      const { getByTestId, rerender } = renderCalendar({
    ...
    test('highlights weekends when passed shouldHighlightWeekends prop', () => {
      ...
      rerender(
        <Calendar locale="fa" value={persianValue} shouldHighlightWeekends />,
      )
  ...
  describe('Days Selection', () => {
    ...
    test('replaces value on new date selection', () => {
      ...
      rerender(
        <Calendar
          onChange={onChange}
          value={{ from: initialValue, to: initialValue }}
        />,
      )
      fireEvent.click(newDayButton)
      expect(onChange).toHaveBeenCalledTimes(2)
  ...
  describe('Custom Days Class Names', () => {
    test('Adds a custom class name to preferred days', () => {
      ...
      const { getDay, standardDays } = renderCalendar({
        ...
        customDaysClassName: [
          { ...customDayToAddClass, className: customClassName },
        ],
      })
  ...
  describe('Month Changes by Month Arrow Buttons', () => {
    test('changes month on arrow click', () => {
      ...
      const hiddenNextMonthDaysSection = findHiddenSliderItem(
      ...
      expect(nextMonthButton.textContent).toBe(
        getMonthName(nextMonthDate.month),
      )
      ...
      const hiddenPreviousMonthDays = standardDays.filter(
        day => !day.classList.contains('-blank'),
      )
  ...
  describe('Minimum & Maximum Dates', () => {
    test('disables days before minimum date and after maximum date', () => {
      ...
      const areAllDisabled = list =>
        list.every(day => day.classList.contains('-disabled'))
